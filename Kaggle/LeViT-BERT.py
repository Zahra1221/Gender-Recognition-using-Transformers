...
text1 = pd.read_csv (path+'/train_text_combined_BERT.csv')
text2 = pd.read_csv (path+'/val_text_combined_BERT.csv')
text3 = pd.read_csv (path+'/test_text_combined_BERT.csv')
...
for epoch in range (epochs):
  ...
  if val_acc > best_accuracy:
    torch.save(imgModel.state_dict(), 'LeViT-BERT')
    best_accuracy = val_acc

#loading the model
imgModel = Combination (dropout= 0.5, hidden= 5)
imgModel = imgModel.to (device)
imgModel.load_state_dict(torch.load('LeViT-BERT'))
