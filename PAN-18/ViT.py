...
validation = torch.utils.data.ConcatDataset(torch.utils.data.Subset(ds, indices[:500]), torch.utils.data.Subset(ds, indices[15000:15500])) # The validation should have 50 female users and 50 male users
train = torch.utils.data.ConcatDataset(torch.utils.data.Subset(ds, indices[500:15000]), torch.utils.data.Subset(ds, indices[15500:]))

label2id = {'F':'0', 'M':'1'}
id2label = {'0':'F', '1':'M'}
feature_extractor = ViTFeatureExtractor.from_pretrained('google/vit-base-patch16-224-in21k')
vit = ViTForImageClassification.from_pretrained('google/vit-base-patch16-224-in21k', num_labels=len(label2id), label2id=label2id, id2label=id2label)
...
class ImageClassifier (nn.Module):
  ...
  def forward (self, input_ids):
    return self.vit (input_ids)

imgModel = ImageClassifier (vit)
...
for epoch in range (epochs):
  ...
  val_acc, val_loss, val_F_Percision, val_M_Percision, val_F_Recall, val_M_Recall = eval_model(imgModel, val_loader, ce_loss, device, len(validation) )
  ...
  if val_acc > best_accuracy:
    torch.save(imgModel.state_dict(), 'ViTModel1')
    best_accuracy = val_acc

# Combining Images
imgModel = ImageClassifier (vit)
imgModel = imgModel.to (device)
imgModel.load_state_dict(torch.load('ViTModel1'))
...
val_concat, val_labels, A, P = TransformerModel (imgModel, val_loader, device)
...
df.to_csv ('ViT_train_combine.csv', index=False, encoding='utf-8')
...
df2.to_csv ('ViT_test_combine.csv', index=False, encoding='utf-8')
...
for epoch in range (epochs):
  ...
  history['val_M_Recall'].append (val_M_Recall)
  if val_acc > best_accuracy:
    torch.save(model.state_dict(), 'ViTModelCombiningImages')
    best_accuracy = val_acc
...
df.to_csv ('ViT_train_final_output.csv', index=False)
P0, P1, A = TheImageCombinedModel (model, val_loader, device)
df = pd.DataFrame (data= {'data0':P0, 'data1':P1, 'Label':A})
df.to_csv ('ViT_validation_final_output.csv', index=False)
P0, P1, A = TheImageCombinedModel (model, test_loader, device)
df = pd.DataFrame (data= {'data0':P0, 'data1':P1, 'Label':A})
df.to_csv ('ViT_test_final_output.csv', index=False)

os.remove ('ViT_train_combine.csv')
os.remove ('ViT_val_combine.csv')
os.remove ('ViT_test_combine.csv')
