...
image1 = pd.read_csv ('leViT_Kaggle_train_final.csv')
image2 = pd.read_csv ('leViT_Kaggle_val_final.csv')
image3 = pd.read_csv ('leViT_Kaggle_test_final.csv')
text1 = pd.read_csv (path+'/roberta_train_text_combined.csv')
text2 = pd.read_csv (path+'/roberta_val_text_combined.csv')
text3 = pd.read_csv (path+'/roberta_test_text_combined.csv')

train = pd.DataFrame (data= {'img0':list(image1['data0']), 'img1':list(image1['data1']), 'img2':list(image1['data2']), 'txt0':list(text1['data0']), 'txt1':list(text1['data1']), 'txt2':list(text1['data2']), 'label':list(image1['Labels'])})
val = pd.DataFrame (data= {'img0':list(image2['data0']), 'img1':list(image2['data1']), 'img2':list(image2['data2']), 'txt0':list(text2['data0']), 'txt1':list(text2['data1']), 'txt2':list(text2['data2']), 'label':list(image2['Labels'])})
test = pd.DataFrame (data= {'img0':list(image3['data0']), 'img1':list(image3['data1']), 'img2':list(image3['data2']), 'txt0':list(text3['data0']), 'txt1':list(text3['data1']), 'txt2':list(text3['data2']) 'label':list(image3['Labels'])})

train_data = torch.tensor (train[['img0','img1', img2,'txt0','txt1', 'txt2']].to_numpy())
train_label = torch.tensor(train[['label']].to_numpy())
val_data = torch.tensor (val[['img0','img1', 'img2','txt0','txt1', 'txt2']].to_numpy())
val_label = torch.tensor(val[['label']].to_numpy())
test_data = torch.tensor (test[['img0','img1','img2','txt0','txt1','txt2]].to_numpy())
...
class Combination (nn.Module):
  def __init__ (self, hidden, dropout, input_size=6, classes=3):
...
for epoch in range (epochs):
  ...
  val_acc, val_loss, val_B_Percision, val_F_Percision, val_M_Percision, val_B_Recall, val_F_Recall, val_M_Recall = eval_model(model, test_loader, ce_loss, device, len(test_ds), best_acc2 )
  ...
  if val_acc > best_accuracy:
    torch.save(imgModel.state_dict(), 'LeViT-RoBERTa')
    best_accuracy = val_acc

#loading the model
imgModel = Combination (dropout= 0.5, hidden= 5)
imgModel = imgModel.to (device)
imgModel.load_state_dict(torch.load('LeViT-RoBERTa'))
