...
from transformers import AutoImageProcessor, LevitForImageClassification, AutoFeatureExtractor, LevitFeatureExtractor
from torch.utils.data import DataLoader, Dataset
from torchvision.datasets import ImageFolder
...
class ImageClassifier (nn.Module):
  def __init__ (self, levit, classes=3):
...
for epoch in range (epochs):
  ...
  val_acc, val_loss, val_B_Percision, val_F_Percision, val_M_Percision, val_B_Recall, val_F_Recall, val_M_Recall = eval_model(imgModel, val_loader, ce_loss, device, len(validation) )
...
d3 = [x for x in train_labels for q in range(0, 8)] # batch_size was 8
df = pd.DataFrame(data={'data0':d0, 'data1':d1, 'data2':d2, 'labels':A1})
df.to_csv ('leViT_Kaggle_train_final.csv', index=False, encoding='utf-8')
...
df1.to_csv ('leViT_Kaggle_val_final.csv', index=False, encoding='utf-8')
...
df2.to_csv ('leViT_Kaggle_test_final.csv', index=False, encoding='utf-8')
