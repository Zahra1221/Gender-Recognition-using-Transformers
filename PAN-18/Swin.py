...
import transformers
from transformers import AutoTokenizer, AutoImageProcessor, AutoModelForSequenceClassification, AdamW, get_linear_schedule_with_warmup, AutoModel, 
...
class ImageClassifier (nn.Module):
  def __init__ (self, swin, classes=2):
    super (ImageClassifier, self).__init__()
    self.swin = swin
  def forward (self, input_ids):
    return output = self.swin (input_ids)
...
df.to_csv ('Swin_train_combine.csv', index=False, encoding='utf-8')
...
df1.to_csv ('Swin_val_combine.csv', index=False, encoding='utf-8')
...
df2.to_csv ('Swin_test_combine', index=False, encoding='utf-8')
...
model = Combine10ImageClassifier (dropout=0.5, hidden = 10)
...
model.load_state_dict(torch.load('SwinModelCombiningImages'))
...
df.to_csv ('Swin_train_final_output.csv', index=False)
P0, P1, A = TheImageCombinedModel (model, val_loader, device)
df = pd.DataFrame (data= {'data0':P0, 'data1':P1, 'Label':A})
df.to_csv ('Swin_validation_final_output.csv', index=False)
P0, P1, A = TheImageCombinedModel (model, test_loader, device)
df = pd.DataFrame (data= {'data0':P0, 'data1':P1, 'Label':A})
df.to_csv ('Swin_test_final_output.csv', index=False)

os.remove ('Swin_train_combine.csv')
os.remove ('Swin_val_combine.csv')
os.remove ('Swin_test_combine.csv')
