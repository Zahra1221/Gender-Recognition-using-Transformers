...
import transformers
from transformers import AutoTokenizer, AutoModelForSequenceClassification, AdamW, get_linear_schedule_with_warmup, AutoModel, RobertaModel, RobertaTokenizer
...
model = GenderClassifier (roberta, classes)
...
for epoch in range (epochs):
  ...
  val_acc, val_loss, val_F_Percision, val_M_Percision, val_F_Recall, val_M_Recall = eval_model(model, val_dataloader, ce_loss, device, len(test_data) )
  ...
  if val_acc > best_accuracy:
    torch.save(model.state_dict(), 'RoBERTaModel')
    best_accuracy = val_acc

#loading the model
model = GenderClassifier (roberta, classes)
model = model.to (device)
model.load_state_dict(torch.load('RoBERTaModel'))
...
df4 = pd.DataFrame (data={'F':f, 'M':m, 'labels':labels})
df4.to_csv (path+'/roberta_test_output.csv')

df1 = pd.read_csv (path+'/roberta_train_output1.csv')
df2 = pd.read_csv (path+'/roberta_train_output2.csv')
df3 = pd.read_csv (path+'/roberta_val_output.csv')
df4 = pd.read_csv (path+'/roberta_test_output.csv')
...
for epoch in range (epochs):
  ...
  print(f'Train loss {train_loss} accuracy {train_acc} Female Percision {train_F_Percision} Male Percision {train_M_Percision} Female Recall {train_F_Recall} Male Recall {train_M_Recall}')
  val_acc, val_loss, val_F_Percision, val_M_Percision, val_F_Recall, val_M_Recall = eval_model(model, val_loader, ce_loss, device, len(test_ds) )
  print(f'Val loss {val_loss} accuracy {val_acc} Female Percision {val_F_Percision} Male Percision {val_M_Percision} Female Recall {val_F_Recall} Male Recall {val_M_Recall}')
  print()
  history['train_acc'].append(train_acc)
  history['train_loss'].append(train_loss)
...
model = TextCombination (5, 0.1)
model = model.to (device)
model.load_state_dict(torch.load('RobertaModelCombiningTexts'))
